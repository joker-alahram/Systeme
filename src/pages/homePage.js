...
function productsByIds(state, ids) {
  const safeProducts = Array.isArray(state.commerce.catalog.products) ? state.commerce.catalog.products.filter((item) => item && item.product_id) : [];
  const map = new Map(safeProducts.map((item) => [String(item.product_id), item]));
...
export function renderHomePage(state) {
  ...
  const topProducts = productsByIds(state, topRows.map((row) => row.product_id));
  const topProductsReady = Boolean(topRows.length);
  const cartCompanyIds = new Set((state.commerce.cart || []).map((item) => String(item.companyId || item.company_id || '')));
  const basketProducts = products.filter((product) => cartCompanyIds.has(String(product.company_id || '')) || (state.commerce.cart || []).some((item) => String(item.id) === String(product.product_id))).slice(0, 6);
  const featuredProducts = topProducts.slice(0, 6);
  const mostRequested = topRows.slice(0, 6).map((row) => products.find((product) => String(product.product_id) === String(row.product_id))).filter(Boolean);
