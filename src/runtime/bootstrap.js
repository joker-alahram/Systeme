...
import { loadHomeCatalog, loadHomeSupplementary, loadTopSections, loadCompanyCatalog, aggregateRuntimeProducts, projectRuntimeProducts } from '../services/catalogService.js';
...
async function ensureCompanyCatalogLoaded(store, api, companyId) {
  ...
  if (isHydrationSessionActive(hydrationToken) && Array.isArray(cachedRows) && cachedRows.length > 0) {
    store.update((draft) => {
  ...
  try {
    ...
    if (companyHydrationTokens.get(trimmed) !== requestToken || !isHydrationSessionActive(hydrationToken)) return;
    ...
    if (isHydrationSessionActive(hydrationToken) && fallbackRows.length) {
      store.update((draft) => {
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-tier') {
      ...
      const homeCatalog = await loadHomeCatalog(api, selectedTier, { includeTop: false, criticalOnly: true });
      ...
      void (async () => {
        try {
          const topSections = await loadTopSections(api);
          const current = store.getState();
          if (normalizeTierName(current.commerce.selectedTier) !== selectedTier) return;
          store.patch({ commerce: { ...current.commerce, catalog: { ...current.commerce.catalog, top: topSections.top } } }, { silent: true });
          schedule('page');
        } catch {
          // ignore top-section refresh failures
        }
      })();
