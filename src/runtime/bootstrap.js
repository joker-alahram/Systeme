...
import { loadHomeCatalog, loadTopSections, loadCompanyCatalog, aggregateRuntimeProducts, projectRuntimeProducts } from '../services/catalogService.js';
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-tier') {
      ...
      const homeCatalog = await loadHomeCatalog(api, selectedTier, { includeTop: false });
...
export async function bootstrapApp() {
  ...
  void (async () => {
    markPerf('boot:catalog:live:start');
    try {
      const liveCatalog = await loadHomeCatalog(api, bootTier, { includeTop: false }).catch(() => null);
      if (!liveCatalog) {
        markPerf('boot:catalog:live:failed');
        return;
      }
      const currentTier = normalizeTierName(store.getState().commerce.selectedTier) || bootTier;
      if (currentTier !== bootTier) {
        markPerf('boot:catalog:live:skipped');
        return;
      }
      ...
      markPerf('boot:catalog:live:success');
      measurePerf('boot:catalog:live', 'boot:catalog:live:start', 'boot:catalog:live:success');
    } catch (error) {
      console.error(error);
      markPerf('boot:catalog:live:failed');
      measurePerf('boot:catalog:live', 'boot:catalog:live:start', 'boot:catalog:live:failed');
    }
  })();
